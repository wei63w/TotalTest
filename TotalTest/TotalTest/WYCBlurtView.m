...
@interface WYCBlurtView ()<UIScrollViewDelegate>
...
- (void)setupContentView {
    ...
    [self.icon sd_setImageWithURL:[NSURL URLWithString:self.imgUrl] placeholderImage:[UIImage imageNamed:@"1"]];
    
    [imageView sd_setImageWithURL:[NSURL URLWithString:self.imgUrl] placeholderImage:[UIImage imageNamed:@"1"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        // 20 左右 R  模糊图片
        [imageView setImageToBlur:imageView.image blurRadius:21 completionBlock:nil];
    }];
